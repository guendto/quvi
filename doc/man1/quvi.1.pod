...
Print path to the config file and exit.
...
Enable category I<arg> webscripts. I<arg> may a list of the categories
to enable, each category separated by a comma (','). I<arg> can also
be C<help>. All of the categories are enabled by default.

=head2 -F, --query-formats

Query available formats to an URL. The returned array contains a
dynamically (queried from the server) created list of the available
formats to the URL.

You can use these I<format strings> with C<--format>.

How the format strings in the array are created is website specific
and determined by the I<libquvi script> responsible of parsing the
media details for the URL.

See also C<--format>.

=head2 -f, --format I<arg> (=default)

Query format I<arg> of the media. The I<arg> can also be C<default>
or C<best>. The I<arg> value is used with B<all> of the URLs fed to
quvi at runtime.

If the I<arg> is C<best>, the I<libquvi script> responsible of parsing
...
URL to whatever was deemed to be the C<default> format to the URL.

The I<libquvi script> will switch to the C<default> format if the I<arg>
was unrecognized or the requested I<arg> format was not available.

For the above reasons, the resulting format may be different from the
